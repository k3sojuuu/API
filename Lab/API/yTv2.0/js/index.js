...
function loadVideo(keyword){
    var YOUTOBE_API = "https://content.googleapis.com/youtube/v3/search?q=" + 
    keyword + "&type=video&maxResults=9&part=snippet&key=AIzaSyCO_f4mE8n4Pw1R0XN3xUZO1tXUAXHuCHw"
    var xhr = new XMLHttpRequest();
    xhr.open("GET", YOUTOBE_API, true);
    xhr.onreadystatechange = function(){
        if(this.readyState == 4 && this.status == 200){
            ...
            for (var i = 0; i < responseJson.items.length; i++){
                if(responseJson.items[i].id.kind == 'youtobe#channel'){
                    continue;
                }
...
function showVideo(videoId){
    videoFrame.src = "https://www.youtobe.com/embed/" + videoId + "?autoplay=1";
    setTimeout(function(){
